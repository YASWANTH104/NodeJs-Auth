...
const uploadImageController=async(req,res)=>{
    try{
        ...
        const newlyUploadedImge=new Image({
            url,
            publicId,
            uploadedBy:req.userInfo.userId
        })
        await newlyUploadedImge.save();

        //delete file from the localStorage
        fs.unlinkSync(req.file.path);

        res.status(201).json({
            success:true,
            message:'Image uploaded Successfully',
            image:newlyUploadedImge,
        })
    }catch(e){
        console.log(e);
        res.status(500).json({
...
const deleteImageController=async(req,res)=>{
    try{
        ...
        //delete this image first from the cloudinary
        await cloudinary.uploader.destroy(image.publicId)
        //delete this image from the mongo db
